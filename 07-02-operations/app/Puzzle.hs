...
import Control.Monad.Amb (Amb, aMemberOf, isPossible)
import GHC.Num.Integer

------- SEARCH --------

data Ops = Plus | Times | Cat
...
composeMember target (i:rest) accumulator = do
    op <- aMemberOf [Plus, Times, Cat]
    newAccumulator <- case op of
        Plus -> return (i+accumulator)
        Times -> return (i*accumulator)
        Cat -> return ( (10^digits i) * accumulator + i )
